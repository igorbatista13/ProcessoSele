<div class="tab-pane" id="profile">
    ...
    <form action="{{ route('perfil.update') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="col-md-3 col-lg-3">
                <select class="form-control" id="sexo" name="sexo">
                    <option value="Masculino"
                        {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->sexo == 'Masculino' ? 'selected' : '' }}>
                        Masculino</option>
                    <option value="Feminino"
                        {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->sexo == 'Feminino' ? 'selected' : '' }}>
                        Feminino</option>
            ...
            <div class="col-md-3 col-lg-3">
                <select class="form-control" id="escolaridade" name="escolaridade">
                    <option value="Fundamental"
                        {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->escolaridade == 'Fundamental' ? 'selected' : '' }}>
                        Fundamental</option>
                    <option value="Ensino Médio"
                        {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->escolaridade == 'Ensino Médio' ? 'selected' : '' }}>
                        Ensino Médio
                    </option>
                    <option value="Ensino Superior (Graduação)"
                        {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->escolaridade == 'Ensino Superior (Graduação)' ? 'selected' : '' }}>
                        Ensino Superior (Graduação)
                    </option>
                    <option value="Pós-graduação"
                    {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->escolaridade == 'Pós-graduação' ? 'selected' : '' }}>
                    Pós-graduação
                    </option>
                    <option value="Mestrado"
                    {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->escolaridade == 'Mestrado' ? 'selected' : '' }}>
                    Mestrado
                        </option>
                    <option value="Doutorado"
                    {{ Auth::check() && Auth::user()->perfil && Auth::user()->perfil->escolaridade == 'Doutorado' ? 'selected' : '' }}>
                    Doutorado
                    </option>
                </select>
            </div>
        ...
        <div class="row mb-3">
            <label for="orgao" class="col-md-4 col-lg-3 col-form-label">Órgão</label>
            <div class="col-md-8 col-lg-9">
